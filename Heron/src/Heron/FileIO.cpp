...
namespace Heron
{
	std::vector<float> FileIO::read_28x28_bmp(const std::string & path) {
		...
		if (infoHeader.biWidth != 28 || infoHeader.biHeight != 28) throw std::runtime_error("BMP ain't 28x28");
		if (infoHeader.biBitCount != 24 || infoHeader.biCompression != 0) throw std::runtime_error("Only 24-bit uncompressed BMP supported");
	...
	void FileIO::write_28x28_bmp(const std::vector<float>& img, const std::string& path) {
