...
namespace Heron
{
	...
	static constexpr size_t ACT_COUNT =
		sizeof(ACT_TABLE) / sizeof(ACT_TABLE[0]);
	...
	void Network::save_model(const std::string& path) const
	{
		...
		for (auto& W : weights) {
			...
			for (auto& row : W)
				out.write((char*)row.data(), c * sizeof(float));
		}
	...
	void Network::predict(
		...
		std::vector<std::vector<float>>& A_out
	) const
	{
		const size_t L = weights.size();
