...
int main() {
  ...
  if (dataset_path.empty()) {
    dataset_path =
        "C:\\Users\\kedar\\source\\repos\\neural_networks\\mnist_nn_scratch\\datasets";
  }
...
restart:
  ...
  if (choice == 1) {
    ...
    net.save_model(dataset_path+"\\model.nn");

    std::cout << "Model saved.\n";
  } else if (choice == 2 || choice == 3 || choice == 4) {
    if (!loader.load(choice == 2 || choice == 3 ? "test" : "")) {
    ...
    std::vector<ActFn> activations = { Activation::relu, Activation::softmax};
    Network net(layer_sizes, activations);
    net.load_model(dataset_path+"\\model.nn");
    ...
    if (choice == 2) {
      ...
      FileIO::write_28x28_bmp(
          X[idx], dataset_path + "/" + std::to_string(idx) + ".bmp");
      std::cout << "Image written to " << idx << ".bmp\n";
    } else if (choice == 4) {
      std::string img_path;
      ...
      FileIO::write_28x28_bmp(img, dataset_path + "/loaded_image.bmp");
  ...
  std::cout << "Would you like to exit [Y\\n]: ";
  char a;
  std::cin >> a;
