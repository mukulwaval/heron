...
#else
std::string Utf8ToNative(const std::string& str) { return str; }
#endif
...
bool PlatformWin32::ApplicationStart(int argc, char** argv) {
  ...
  auto winProc = [](HWND hWnd, UINT msg, WPARAM wParam,
                    LPARAM lParam) -> LRESULT {
    return s_Instance->WinProc(hWnd, msg, wParam, lParam);
  };

  m_WindowClass = {sizeof(WNDCLASSEX),
                   CS_CLASSDC,
                   winProc,
                   0L,
                   0L,
...
bool PlatformWin32::OpenMainWindow(const char* title, int width, int height) {
  ...
  m_MainWindowHandle = CreateWindow(
      m_WindowClass.lpszClassName, Utf8ToNative(title).c_str(),
      WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
      width < 0 ? CW_USEDEFAULT : width, height < 0 ? CW_USEDEFAULT : height,
...
bool PlatformWin32::CloseMainWindow() {
  if (m_MainWindowHandle == nullptr) return true;
...
void* PlatformWin32::GetMainWindowHandle() const { return m_MainWindowHandle; }
...
void PlatformWin32::ShowMainWindow() {
  if (m_MainWindowHandle == nullptr) return;
...
bool PlatformWin32::ProcessMainWindowEvents() {
  if (m_MainWindowHandle == nullptr) return false;
...
void PlatformWin32::SetRenderer(Renderer* renderer) { m_Renderer = renderer; }
...
void PlatformWin32::Quit() { PostQuitMessage(0); }
...
LRESULT PlatformWin32::WinProc(HWND hWnd, UINT msg, WPARAM wParam,
                               LPARAM lParam) {
  ...
  switch (msg) {
    ...
    case WM_SIZE:
      ...
      if (m_Renderer != nullptr && wParam != SIZE_MINIMIZED)
        m_Renderer->Resize(static_cast<int>(LOWORD(lParam)),
