...
#if defined(RENDERER_IMGUI_OGL3)
...
void RendererOpenGL3::NewFrame() { ImGui_ImplOpenGL3_NewFrame(); }
...
ImTextureID RendererOpenGL3::CreateTexture(const void* data, int width,
                                           int height) {
  ...
  return static_cast<ImTextureID>(
      static_cast<std::uintptr_t>(texture.TextureID));
}

ImVector<ImTexture>::iterator RendererOpenGL3::FindTexture(
    ImTextureID texture) {
  auto textureID = static_cast<GLuint>(static_cast<std::uintptr_t>(texture));
...
void RendererOpenGL3::DestroyTexture(ImTextureID texture) {
  ...
  if (textureIt == m_Textures.end()) return;
...
int RendererOpenGL3::GetTextureWidth(ImTextureID texture) {
  auto textureIt = FindTexture(texture);
  if (textureIt != m_Textures.end()) return textureIt->Width;
  return 0;
}
...
void RendererOpenGL3::BeginImGuiPlatformWindows() {
  glGetIntegerv(GL_FRAMEBUFFER_BINDING, &m_BackupFBO);
  glGetIntegerv(GL_VIEWPORT, m_BackupViewport);
}

void RendererOpenGL3::EndImGuiPlatformWindows() {
  glBindFramebuffer(GL_FRAMEBUFFER, m_BackupFBO);
  glViewport(m_BackupViewport[0], m_BackupViewport[1], m_BackupViewport[2],
             m_BackupViewport[3]);
}
