...
struct RendererDX11 final : Renderer {
...
bool RendererDX11::Create(Platform& platform) {
  ...
  auto hr =
      CreateDeviceD3D(reinterpret_cast<HWND>(platform.GetMainWindowHandle()));
...
void RendererDX11::NewFrame() { ImGui_ImplDX11_NewFrame(); }
...
void RendererDX11::Clear(const ImVec4& color) {
  m_deviceContext->ClearRenderTargetView(m_mainRenderTargetView,
                                         (float*)&color.x);
}

void RendererDX11::Present() { m_swapChain->Present(1, 0); }

void RendererDX11::Resize(int width, int height) {
  ImGui_ImplDX11_InvalidateDeviceObjects();
  CleanupRenderTarget();
  m_swapChain->ResizeBuffers(0, (UINT)width, (UINT)height, DXGI_FORMAT_UNKNOWN,
                             0);
  CreateRenderTarget();
}
...
HRESULT RendererDX11::CreateDeviceD3D(HWND hWnd) {
  ...
  if (D3D11CreateDeviceAndSwapChain(
          ...
          featureLevelArray, 1, D3D11_SDK_VERSION, &sd, &m_swapChain, &m_device,
          &featureLevel, &m_deviceContext) != S_OK)
    return E_FAIL;
...
void RendererDX11::CreateRenderTarget() {
  ...
  m_device->CreateRenderTargetView(pBackBuffer, &render_target_view_desc,
...
ImTextureID RendererDX11::CreateTexture(const void* data, int width,
                                        int height) {
  ...
  m_device->CreateTexture2D(&desc, &sub, &texture);
  m_device->CreateShaderResourceView(texture, nullptr, &srv);
