...
void PlatformGLFW::ApplicationStop() { glfwTerminate(); }
...
bool PlatformGLFW::OpenMainWindow(const char* title, int width, int height) {
  ...
  glfwSetWindowCloseCallback(m_Window, [](GLFWwindow* window) {
    ...
    if (!self->m_QuitRequested) self->CloseMainWindow();
  });
...
void PlatformGLFW::SetRenderer(Renderer* renderer) { m_Renderer = renderer; }
