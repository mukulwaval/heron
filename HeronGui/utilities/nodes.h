...
struct NodeEditor : ImFlow::BaseNode {
	...
	NodeEditor(float size) : ImFlow::BaseNode() {
		clear();
	}
	...
	void loadGraph(const SavedGraph& graph)
	{
		...
		for (const SavedLink& sl : graph.links)
		{
			...
			if (itFrom == uidToNode.end() || itTo == uidToNode.end())
				continue;
		...
		for (auto& [uid, node] : uidToNode)
		{
			...
			for (auto& inPin : node->getIns())
			{
				...
				if (weakLink.lock().get())
					weakLink.lock().get()->update();
			}
...
private:
	...
	void linkDropped(ImFlow::Pin* droppedFrom) {
		...
		if (!ImGui::IsPopupOpen("Background Context Menu")) ImGui::OpenPopup("Background Context Menu");
		drawBackgroundContextMenu();
	}
	...
	void clear() {
		...
		mINF.setSize(mSize);
		mINF.droppedLinkPopUpContent([this](ImFlow::Pin* fromPin) { linkDropped(fromPin); });
	}
