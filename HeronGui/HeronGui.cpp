...
struct MainEditor : public Application {
  ...
  void OnFrame(float deltaTime) override {
...
#ifdef __linux__
    static char imgPath[256] = "images/test_index.bmp";
#endif
    ...
    Components::Dockspace([&]() {
      ...
      ImGui::SetNextItemWidth(-1);
      ImGui::InputText("##eval_model_path", evalModelPath,
                       sizeof(evalModelPath));
      ...
      if (evalRunning)
        ImGui::TextColored(ImVec4(1, 1, 0, 1), "Status: %s", "Evaluating...");
      if (evalDone)
        ImGui::TextColored(ImVec4(0, 1, 0, 1), "Status: %s", "Done");
      else
      ...
      if (!imgPredRunning && ImGui::Button("Predict Image", ImVec2(-1, 0))) {
        ...
        std::thread([this, savedImgPath, savedModelPath]() {
          ...
          try {
            ...
            for (int i = 0; i < 10; ++i) imgClassProbs[i] = out[i] / sum;
      ...
      if (imgPredRunning)
        ImGui::TextColored(ImVec4(1, 1, 0, 1), "Status: %s", "Running...");
      if (imgPredDone)
        ImGui::TextColored(ImVec4(0, 1, 0, 1), "Status: %s", "Done");
      else
        ImGui::Text("Status: %s", "Idle");
      ...
      if (imgPredDone && imgPredResult.load() != -1) {
        ImGui::TextColored(ImVec4(0, 1, 0, 1), "Prediction: %d",
                           imgPredResult.load());
      }
        ...
        if (ImGui::BeginTable("##class_probs", 3,
          ...
          for (int i = 0; i < 10; ++i) {
            ...
            ImGui::PushStyleColor(ImGuiCol_PlotHistogram,
                                  ImVec4(0.5f, 0.5f, 0.5f, 1.0f));  // fill
      ...
      if (!imgPreview.empty()) {
        ...
        for (int y = 0; y < 28; ++y) {
          for (int x = 0; x < 28; ++x) {
            ...
            ImVec2 a(origin.x + x * pixel, origin.y + y * pixel);
            ImVec2 b(a.x + pixel, a.y + pixel);
      ...
      if (!sampleRunning && ImGui::Button("Predict Sample", ImVec2(-1, 0))) {
        ...
        std::thread([this, savedIdx, savedModelPath]() {
          ...
          for (int i = 0; i < 10; ++i) sampleClassProbs[i] = out[i] / sum;
...
#ifdef _WIN32
          Heron::FileIO::write_28x28_bmp(X[savedIdx], "images\\test_index.bmp");
          log("[SAMPLE] Image saved to images\\test_index.bmp");
#endif

#ifdef __linux__
          Heron::FileIO::write_28x28_bmp(X[savedIdx], "images/test_index.bmp");
          log("[SAMPLE] Image saved to images/test_index.bmp");
#endif
      ...
      if (sampleDone && samplePred.load() != -1) {
        ImGui::TextColored(ImVec4(0, 1, 0, 1), "Prediction: %d",
                           samplePred.load());
        ImGui::Text("Label: %d", sampleLabel.load());
      }
...
#ifdef __linux__
          previewImg = Heron::FileIO::read_28x28_bmp("images/test_index.bmp");
#endif
        } catch (...) {
      ...
      if (model) {
        ...
        for (size_t i = 0; i < model->activations.size(); ++i) {
          ImGui::BulletText(
              "Layer %zu: %s", i,
              model->activations[i].name ? model->activations[i].name : "None");
        }
      ...
      if (buildRequested && model && !training) {
        ...
        std::vector<std::vector<float> (*)(const std::vector<float>&)>
            savedActivations;
        ...
        std::thread([this, savedPath, savedLayers, savedActivations, savedIters,
                     savedLr]() {
          auto log = [&](const std::string& msg) {
          ...
          Heron::Trainer::gradient_descent(
              net, X, Y, savedLr, savedIters, [this](int iter, float acc) {
                {
                  std::lock_guard lock(progressMutex);
                ...
                {
                  std::lock_guard lock(logMutex);
                  logs.push_back("[ITER " + std::to_string(iter) +
      ...
      if (training)
        ImGui::TextColored(ImVec4(1, 1, 0, 1), "Status: %s", "Training...");
      if (trainingDone)
        ImGui::TextColored(ImVec4(0, 1, 0, 1), "Status: %s", "Done");
      else
        ImGui::Text("Status: %s", "Idle");
